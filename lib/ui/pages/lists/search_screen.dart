...
import '../generics/any_item.dart';
import '../words/word_item.dart';
import '../trivia/trivia_menu.dart';
...
final filtersTable = [
  DbUtils.wordsTable,
  DbUtils.idiomsTable,
  DbUtils.sayingsTable,
  DbUtils.proverbsTable
];
    ...
    if (activeTable == filtersTable[0]) {
      ...
      dbFuture.then((database) {
        Future<List<Item>> itemListFuture =
            db.getItemSearch(_letter, activeTable, true);
        itemListFuture.then((resultList) {
          setState(() {
            items = resultList;
            loader.hide();
                ...
                return InkWell(
                  ...
                  child: CustomAnimation(
                    ...
                    tween: ColorTween(
                        ...
                        end: ColorUtils.black.withOpacity(0.08)),
                    builder: (ctx, child, animation) => Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        boxShadow: [
                          BoxShadow(
                            color: isActive
                                ? ColorUtils.black.withOpacity(0.25)
                                : Colors.transparent,
                            blurRadius: 5,
                            offset: Offset(0, 3),
                          ),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          return activeTable == filtersTable[0]
              ? WordItem('WordIndex_' + words[index].id.toString(),
                  words[index], context)
              : AnyItem('ItemIndex_' + items[index].id.toString(), items[index],
                  activeTable, context);
        },
      ),
    );
